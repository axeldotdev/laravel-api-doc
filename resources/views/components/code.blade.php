...
<section x-data="{language: 'curl'}" class="relative w-1/2 px-8 pt-20 pb-12 bg-code text-gray-50 font-mono border-b border-gray-900">
    ...
    @if(!is_null($curl) || !is_null($php) || !is_null($javascript))
    <button x-data @click="copyCode(`#id_{{ $id }}_${language}`); $tooltip('Copied!');" †ype="button" class="absolute top-4 right-8 flex justify-center items-center rounded-lg bg-gray-600 text-gray-400 hover:text-gray-50 transition-colors p-2">
